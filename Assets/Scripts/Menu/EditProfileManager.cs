...
public class EditProfileManager : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        backButton = root.Q<Button>("CancelButton");
        backButton.clicked += OnCancelClicked;

        createButton = root.Q<Button>("EditButton");
    ...
    IEnumerator Edit()
    {
        ...
        form.AddField("currentUsername", DBManager.username);
        ...
        using (UnityWebRequest request = UnityWebRequest.Post("http://localhost/SQLConnect/editprofile.php", form))
        {
            ...
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                if (response == "0")
                {
                    ...
                    successLabel.text = "Account edited successfully!";
                ...
                {
                    Debug.Log("User edit failed. Error #" + response);
                    successLabel.text = "Failed to edit user";
                    successLabel.style.visibility = Visibility.Visible;
                }
            }
            else
